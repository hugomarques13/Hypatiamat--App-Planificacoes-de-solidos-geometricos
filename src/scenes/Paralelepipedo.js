export default class Paralelepipedo extends Phaser.Scene {
  ...
  create() {
    ...
    const titleText = this.add.text(this.scale.width / 2, 20, 'Paralelepípedo', {
      fontFamily: 'Snap ITC',
      fontSize: '40px',
      color: '#ffffff',
      fontStyle: 'bold'
  ...
  onWindowResize() {
    ...
    if (this.unfoldSliderContainer) {
        ...
        this.unfoldSliderContainer.style.left = `${canvasBounds.left + (canvasBounds.width - sliderWidth - rightOffset)}px`;
        this.unfoldSliderContainer.style.top = `${canvasBounds.top + topOffset}px`;
        this.unfoldSliderContainer.style.width = `${sliderWidth}px`;
        this.unfoldSliderContainer.style.padding = `${padding}px`;
        this.unfoldSliderContainer.style.borderRadius = `${padding}px`;
        
        // Update all slider elements
        const sliders = this.unfoldSliderContainer.querySelectorAll('.custom-slider');
        const labels = this.unfoldSliderContainer.querySelectorAll('.slider-label');
